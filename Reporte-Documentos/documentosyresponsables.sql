SELECT
 /** 
            ...
            Modificaciones: 
            26-05-2022. Andrés Del Río. Ajuste de query para calcular vencimiento de acuerdo a fecha de la revisión vigente y no de la fecha de validez del sistema. Esto como medida paliativa mientras se revisa la seguridad de las categorías.
**/
...
CASE WHEN  REVVIG.FECHA_VALIDEZ_CON_REVISION IS NULL
    ...
    THEN 'Vigente'
     WHEN  REVVIG.FECHA_VALIDEZ_CON_REVISION >= <!%TODAY%> AND  REVVIG.FECHA_VALIDEZ_CON_REVISION < <!%TODAY%> + 5
    THEN 'Próximo al Vencimiento'
    WHEN   REVVIG.FECHA_VALIDEZ_CON_REVISION < <!%TODAY%>
    THEN 'Vencido'
END VALIDEZ_REV_VIG,
ROUND(SYSDATE - REVVIG.FECHA_VALIDEZ_CON_REVISION) DIAS_VENCIMIENTO,
